...
WORKDIR /src
COPY ["C#-Problems.csproj", "."]
RUN dotnet restore "./C#-Problems.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./C#-Problems.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./C#-Problems.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "C#-Problems.dll"]